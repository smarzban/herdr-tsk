...
**Scope.** At 110 usable columns or wider, the board becomes a wide split view: the board fills a left allocation and the selected task's complete task view sits in a fully bordered right allocation. The allocations touch with no gap, and the task's left border is the sole visual center separator. The board owns focus initially; changing its selection updates the task side without opening an inline peek. `Enter` or `→` moves focus from the board to task view, while task-view `Esc` or `←` returns focus to the board. The focused task side follows the same view, edit, step, scroll, mouse, verb, save, and refusal behavior as the existing full-page task view. The task border and title are cyan plus bold with task focus and dark gray plus dim with board focus, while content remains monochrome.
...
**Chosen approach.** One responsive board surface with one focus owner and two presentations, over a separate read-only preview or a manually toggled panel. The wide task side is the task view itself, so resizing changes layout rather than changing the user's task session. A full-width board allocation beside one bordered task allocation gives both queue titles and task content room as the frame grows without introducing resize controls or saved layout state.

**Resolved key decisions.** Wide activates at 110 usable columns, inclusive, with no separate height trigger beyond the product's existing operability floor. The full frame is divided between touching left and right allocations whose widths differ by at most one column. The board paints across the full left allocation; the task paints inside the bordered right allocation. Board focus is the initial state. `Enter`/`→` enters task focus; task-view `Esc`/`←` returns to board focus, while an active editor retains its existing key handling. Wide selection paints no inline peek. Resize preserves selection, scroll, focus, task-page state, drafts, and cursor. A dirty task edit locks task switching and explains the refusal. Mouse interaction follows the focused surface and the existing task-page controls.
...
1. **Responsive layout resolver**: owns the pure mapping from usable frame dimensions and focused surface to one of three presentations: single board, single task, or wide split. Contract: input is width, height, and focused surface; output is bounded board and task allocations, the inset task content rectangle, and a shared internal density. At 110 columns or wider it divides all columns equally between touching allocations, with at most one column of difference. The board content rectangle equals the full left allocation; the task content rectangle is inset by its border. In split view both surfaces use the density selected by the narrower content rectangle, so an odd-width split cannot make one side standard while the other remains compact. It returns bounded zero-area rectangles rather than errors at pathological dimensions and never changes session or domain state.
