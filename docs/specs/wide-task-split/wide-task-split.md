...
**Scope.** At 110 usable columns or wider, the board becomes a wide split view: the board on the left and the selected task's complete task view on the right, divided equally around a one-column separator and scaling with the available width. The board owns focus initially; changing its selection updates the task side without opening an inline peek. `Enter` or `→` moves focus from the board to task view, while task-view `Esc` or `←` returns focus to the board. The focused task side follows the same view, edit, step, scroll, mouse, verb, save, and refusal behavior as the existing full-page task view.
...
**Chosen approach.** One responsive board surface with one focus owner and two presentations, over a separate read-only preview or a manually toggled panel. The wide task side is the task view itself, so resizing changes layout rather than changing the user's task session. An equal split gives both queue titles and task content room as the frame grows without introducing divider controls or saved layout state.

**Resolved key decisions.** Wide activates at 110 usable columns, inclusive, with no separate height trigger beyond the product's existing operability floor. The split is equal after reserving one divider column. Board focus is the initial state. `Enter`/`→` enters task focus; task-view `Esc`/`←` returns to board focus, while an active editor retains its existing key handling. Wide selection paints no inline peek. Resize preserves selection, scroll, focus, task-page state, drafts, and cursor. A dirty task edit locks task switching and explains the refusal. Mouse interaction follows the focused surface and the existing task-page controls.

**Glossary terms touched.** wide split view, single-pane view, focused surface (mirrored into `CONTEXT.md`).
...
**AC-3** Wide split view reserves one divider column and divides every other usable column so the two surface widths differ by at most one column. *(Verification type: **test-backed**: property)*
...
- NC-1: No draggable divider or ratio-setting control.
...
1. **Responsive layout resolver**: owns the pure mapping from usable frame dimensions and focused surface to one of three presentations: single board, single task, or wide split. Contract: input is width, height, and focused surface; output is bounded board, task, and optional one-column-divider rectangles plus a shared internal density. At 110 columns or wider it divides the non-divider width equally, with at most one column of difference. In split view both surfaces use the density selected by the narrower rectangle, so an odd-width split cannot make one side standard while the other remains compact. It returns bounded zero-area rectangles rather than errors at pathological dimensions and never changes session or domain state.
2. **Surface focus controller**: owns which surface receives input, independently of task-page edit mode. Contract: input is the current focus, resolved presentation, and a focus-transfer request; output is board or task focus. Shrinking changes presentation but not focus; growing restores split with the same focus. Board-to-task transfers are accepted from board view; task-to-board keyboard transfers are accepted only from task view mode, so an active editor keeps its existing key semantics. It never creates, saves, cancels, or discards task state.
...
**T-1: Define the responsive split geometry contract.** Add a pure resolver that maps usable frame dimensions and retained focus to single-board, single-task, or equal wide-split geometry. At 110 columns and above, reserve one divider column before splitting the remainder, keep the two widths within one column, and choose one shared internal density from the narrower half. Keep the existing standard/compact resolver unchanged for ordinary single-surface callers.

- Files: `src/ui/tier.rs`, `src/ui/board/mod.rs`, `tests/wide_task_split.rs` (new), `CONTEXT.md`.
- Test first: add `wide_geometry_activates_at_110_and_109_stays_single`, `wide_geometry_reserves_one_divider_and_balances_remaining_columns`, `wide_geometry_uses_the_narrower_half_for_shared_density`, and `wide_geometry_is_bounded_across_supported_sizes`; run them against the pre-change resolver and observe the missing geometry contract fail before implementation.
...
**T-2: Render a bounded read-only wide split.** Consume T-1 geometry to paint the ordinary board beside the selected task's existing read-only task-page surface. Paint the divider, translate and clip hit/copy regions to their owning rectangles, suppress only wide-mode peek, and paint an inert empty task side when selection is absent. Preserve all existing zero-origin rendering below 110 columns.
