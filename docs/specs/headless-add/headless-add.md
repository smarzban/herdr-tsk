...
- Initial code set, closed by construction and stable once shipped (may grow): `empty-title` (title absent, blank, or whitespace-only without a C0 control), `invalid-title` (title contains CR, LF, or any other C0 control character, U+0000–U+001F; this check wins before trimming), and `invalid-item` (array element is not an object; `title` is present but not a string; or a known field `notes`/`project` has a wrong type). `notes: null` means absent. Unknown fields do not invalidate an item. Per-item, so one malformed element never discards the plan. Unparseable JSON or a non-array is exit 2. An ambiguous or unknown project falls back verbatim. Store I/O is exit 3 (commit indeterminate).
