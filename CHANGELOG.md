...
- `ctrl+q` quits from every non-editor board surface, including task pages and the wide project preview. `Esc` closes layers, then quits without confirmation at the full-board root on any tab. Unsaved drafts still block quitting, including drafts parked outside the focused preview; text entry and save recovery keep their existing behavior.
