...
fn config_path() -> io::Result<PathBuf> {
    ...
    let root = env::var_os("XDG_CONFIG_HOME")
        ...
        .or_else(|| {
            ...
            #[cfg(windows)]
            {
                // Herdr on Windows reads %APPDATA%\herdr\config.toml, not
                // %USERPROFILE%\.config\herdr (the Unix path).
                env::var_os("APPDATA")
                    .filter(|v| !v.is_empty())
                    .map(PathBuf::from)
            }
...
fn require_min_herdr(version_output: &str) -> io::Result<()> {
    ...
    // A pre-release of the minimum version is accepted: 0.9.0-preview meets the
    // 0.9.0 floor. Older cores (0.8.x) are still rejected, as are pre-releases of
    // older cores (0.8.0-preview).
