...
Editing keys take precedence over view-mode status shortcuts. In the step editor, `ctrl+d`, `ctrl+o`, and `ctrl+x` still address the task; `ctrl+a` adds another step. Notes are reached with `ctrl+e`, then `Tab`. `Alt+Enter` does not save the task edit.
