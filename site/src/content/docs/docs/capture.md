...
Press `Tab` from quick-add to add notes, steps, a thread, or a scope. The draft uses the full pane and opens in Notes. Its forward field ring is Notes → Thread → Scope → Notes, while `Shift+Tab` from Notes reaches Title. In a project preview, its title stays in the right-column header while Notes is active.
