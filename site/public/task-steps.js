...
export class TaskSteps {
  ...
  reset() { this.taskId = null; this.selected = null; this.editor = null; this.staged = null; this.marked = null; this.refusal = ''; }
  bind(task) { if (this.taskId !== task.id) { this.reset(); this.taskId = task.id; } task.steps ??= []; }
  rows(task) { this.bind(task); return this.staged ?? task.steps; }
  get dirty() { return this.staged !== null; }
  move(task, delta) {
  ...
  begin(task, id = null) {
    ...
    this.editor = { id, text: id ? this.rows(task).find(step => step.id === id).text : '' };
    this.refusal = ''; this.marked = null;
  }
  save(task, finish) {
    if (this.editor) {
      const text = this.editor.text.trim();
      if (!text) { this.refusal = 'text required'; return false; }
      if (this.editor.id) {
    ...
    if (finish && this.staged) { task.steps = this.staged; this.staged = null; }
    this.refusal = ''; return true;
  }
  cancel() {
    if (this.editor) { this.editor = null; this.refusal = ''; }
  ...
  remove(task) {
    if (!this.rows(task).some(step => step.id === this.selected)) return false;
    if (this.marked !== this.selected) { this.marked = this.selected; return false; }
