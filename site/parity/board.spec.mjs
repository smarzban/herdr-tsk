...
async function open(page, width) {
 ...
 await page.goto('/');
 await page.locator('#tsk-demo').evaluate((el,w)=>el.style.width=`${w}ch`,width);
 await expect(row(page,12)).toBeVisible();
 await page.locator('#board-demo').focus();
}
async function capture(page,testInfo,name) {
 await page.locator('#tsk-demo').screenshot({path:testInfo.outputPath(`${name}.png`),animations:'disabled'});
}
for (const width of [40,78,109,110]) {
 test(`desk start open back and wrapping at ${width} columns`,async({page},info)=>{
  ...
  const lines = await row(page,13).locator('.tsk-title-line').allTextContents();
  expect(lines.length).toBeGreaterThan(1);
  const appLines = JSON.parse(await readFile(new URL(`../parity-reference/title-${width}.json`,import.meta.url)));
  expect(lines.map(line=>line.trimEnd())).toEqual(appLines);
  expect(lines.join('')).toBe('Read every word of this long title before starting the task so the continuation must align beneath its title and retain the final boundary marker END');
  const bounds=await row(page,13).evaluate(el=>{
   const r=el.getBoundingClientRect();
   return [...el.querySelectorAll('.tsk-title-line')].map(line=>{ const range=document.createRange();range.selectNodeContents(line); const b=range.getBoundingClientRect();return {left:b.left-r.left,right:r.right-b.right}; });
  });
  expect(Math.max(...bounds.map(b=>b.left))-Math.min(...bounds.map(b=>b.left))).toBeLessThan(1);
...
for(const width of [40,78,109]) test(`peek attribution at ${width}`,async({page},info)=>{
 await open(page,width);
 await page.keyboard.press('ArrowRight');
 await expect(page.locator('.tsk-attribution')).toHaveText('    └─ tsk-parity');
 ...
 await page.keyboard.press('ArrowDown');
 await page.keyboard.press('ArrowRight');
 await expect(page.locator('.tsk-attribution')).toHaveText('    └─ #release');
...
test('landing column readout excludes board padding',async({page})=>{
 ...
 await page.evaluate(()=>{
  ...
  split.dataset.split=''; split.style.width='1200px';
  frame.before(split); split.append(frame);
  const readout=document.createElement('span');readout.dataset.cols='';split.append(readout);
