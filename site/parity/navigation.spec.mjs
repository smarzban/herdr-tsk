...
test('project navigation, counted views and selection match the app',async({page})=>{
 ...
 for (const inactive of await page.locator('.tsk-tab-group:not(.is-on)').all()) {
  await expect(inactive).toHaveCSS('text-decoration-line','none');
  const colors = await inactive.evaluate(el => [getComputedStyle(el).color, getComputedStyle(document.querySelector('.tsk-tab-group.is-on')).color]);
...
test('full terminal opens with board and task side by side', async ({page}) => {
 ...
 await expect(page.locator('[data-layout]').first()).toHaveAttribute('data-layout','full');
 await expect(page.locator('[data-layout="full"]')).toHaveAttribute('aria-pressed','true');
...
for (const scope of ['project','projects']) test(`done drawer keeps the thread in ${scope}`, async ({page}) => {
 await page.goto('http://127.0.0.1:4180/');
 await page.locator(`[data-tab="${scope}"]`).click();
 await page.locator('[data-filter]').click();
 await page.locator('[data-filter-option]').filter({hasText:'#auth'}).click();
 await page.locator('#board-demo').focus();
 await page.keyboard.press('z');
 await expect(page.locator('[data-task]')).toHaveCount(3);
 await expect(page.getByRole('button',{name:/Add a health check/})).toHaveCount(0);
});

test('project counts, thread labels and counted menu order match the app', async ({page}) => {
 await page.goto('http://127.0.0.1:4180/');
 await page.locator('[data-tab="projects"]').click();
 const project = page.locator('[data-project-row="launchpad"]');
 await expect(project.locator(':scope > span').nth(1)).toHaveText('#api #auth');
 ...
 await page.locator('[data-tab="project"]').click();
 await page.locator('[data-filter]').click();
 await expect(page.locator('[data-filter-option]').nth(1)).toContainText('#auth  3');
 await expect(page.locator('[data-filter-option]').nth(2)).toContainText('#api  2');
});
