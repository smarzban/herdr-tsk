...
const BOOTSTRAP =
  "Use tsk for my task board. Run `tsk guide` and follow it; if tsk is not installed, read https://gettsk.sh/docs/agents.md and ask me before installing.";
...
test("landing_agents_band_after_cli_copies_bootstrap_and_stays_on_palette", async ({
  ...
  await band.getByRole("button", { name: "Copy bootstrap prompt" }).click();
  const copied = await page.evaluate(() => navigator.clipboard.readText());
