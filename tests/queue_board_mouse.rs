...
#[test]
fn clicking_the_ctrl_g_verb_chip_toggles_the_archived_group() {
    let mut domain = DomainState::new();
    let live = domain
        .create(
            "live row",
    ...
    let region = verb_hit_for_chord(&model, &hits, "g");
    let intent = click(region, &model, &hits).expect("the chip maps to an intent");
    assert_eq!(intent, BoardIntent::ToggleArchivedGroup);
    apply_intent(&mut domain, &mut model, intent, None).expect("toggle applies");
    assert!(
        model.visible_ids().contains(&filed),
        "the click expanded the archived group"
    );
}
