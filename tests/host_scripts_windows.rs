...
fn manifest_actions_point_at_launcher_scripts() {
    ...
    assert!(
        open_board.contains(r#"command = ["pwsh", "-NoProfile", "-File", "scripts/open-board.ps1"]"#),
        "open-board-windows action command must be [\"pwsh\", \"-NoProfile\", \"-File\", \"scripts/open-board.ps1\"] \
    ...
    assert!(
        quick_capture.contains(r#"command = ["pwsh", "-NoProfile", "-File", "scripts/open-capture.ps1"]"#),
        "quick-capture-windows action command must be [\"pwsh\", \"-NoProfile\", \"-File\", \"scripts/open-capture.ps1\"] \
         (not merely a path mention in a comment or elsewhere in the file)"
    );
    assert!(
